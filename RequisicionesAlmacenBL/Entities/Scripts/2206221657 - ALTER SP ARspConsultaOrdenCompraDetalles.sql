...
FROM tblOrdenCompra AS oc
     INNER JOIN tblOrdenCompraDet AS detalle ON oc.OrdenCompraId = detalle.OrdenCompraId AND (oc.Status = 'C' OR detalle.Status != 'C')
	 ...
	 INNER JOIN ARtblAlmacenProducto AS ap ON detalle.CuentaPresupuestalEgrId = ap.CuentaPresupuestalId AND oc.AlmacenId = ap.AlmacenId AND detalle.ProductoId = ap.ProductoId
	 LEFT JOIN
	 (
