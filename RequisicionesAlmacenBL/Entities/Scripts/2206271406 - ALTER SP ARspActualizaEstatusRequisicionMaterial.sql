...
BEGIN
		...
		-- Si todos los detalles no cancelados tienen estatus Relacionado a RC la Requisición pasa a Requisición de Compra
		ELSE IF ( @contador_registros = @DETALLE_RELACIONADO_RC )
		BEGIN
				SET @estatusNuevoId = @ESTATUS_REQUISICION_COMPRA
		END

		-- Si todos los detalles no cancelados tienen estatus En Almacén la Requisición pasa a En Almacén
		ELSE IF ( @contador_registros = @contador_En_almacen )
		BEGIN
				SET @estatusNuevoId = @ESTATUS_EN_ALMACEN
		END
...
UPDATE ARtblRequisicionMaterial SET EstatusId = ISNULL(@estatusNuevoId, EstatusId), FechaUltimaModificacion = GETDATE() WHERE RequisicionMaterialId = @requisicionId