...
BEGIN
		...
		SELECT almacenProducto.AlmacenProductoId,
				...
				detalle.Cantidad * CASE WHEN detalle.TipoMovimientoId = @DISMINUYE THEN -1 ELSE 1 END AS CantidadMovimiento,
				concepto.ConceptoAjuste AS MotivoMovto,
		...
		FROM ARtblInventarioAjuste AS inventario
				...
				INNER JOIN ARtblControlMaestroConceptoAjusteInventario AS concepto ON detalle.ConceptoAjusteId = concepto.ConceptoAjusteInventarioId
