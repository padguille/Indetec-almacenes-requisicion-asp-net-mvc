...
FROM ARtblRequisicionMaterial AS requisicion   
	 ...
	 (
		...
		WHERE EstatusId IN (88, 84, 90, 83, 80) -- Revisión, Por surtir, Surtido parcial, Por Comprar, En almacén
		GROUP BY RequisicionMaterialId
	 ) AS detalles ON requisicion.RequisicionMaterialId = detalles.RequisicionMaterialId
WHERE requisicion.EstatusId IN (64, 68, 67) -- Autorizada, En Proceso, En almacén
GO