...
BEGIN
		...
		DECLARE @51001 VARCHAR(MAX) = @mensaje + 'Existe un inventario iniciado para el almacén [@Almacen].'
		DECLARE @51002 VARCHAR(MAX) = @mensaje + 'Es necesario realizar la Configuración de Fechas correspondiente.'
		DECLARE @51003 VARCHAR(MAX) = @mensaje + 'La Fecha de Operación no puede ser menor al @FechaMinima'
		DECLARE @51004 VARCHAR(MAX) = @mensaje + 'No existe un periodo abierto para realizar el movimiento.'

		-- Obtenemos la Fecha de Operación para registrar los movimientos
		...
		DECLARE @fechaOperacion DATETIME = ( CONVERT(VARCHAR(100), @fechaSistema) + ' ' + CONVERT(VARCHAR(8), GETDATE(), 108) )
		DECLARE @fechaMinima DATETIME = ( SELECT MAX(FechaCreacion) FROM ARtblInventarioMovimiento )
		DECLARE @periodoAbierto INT = ( SELECT COUNT(Ejercicio) FROM tblAdministracionPeriodo WHERE Ejercicio = YEAR(@fechaOperacion) AND Mes = MONTH(@fechaOperacion) AND TipoPoliza = 'A' AND Status = 1 )
		...
		ELSE IF ( @fechaMinima IS NOT NULL AND @fechaOperacion < @fechaMinima )
		BEGIN 
			SET @mensaje = REPLACE(@51003, '@FechaMinima', CONVERT(VARCHAR, @fechaMinima, 3));
		...
		BEGIN				
				...
				--Buscamos si existe un Inventario Físico iniciado
				...
				WHERE inventario.EstatusId = 32 -- En Proceso
					  AND inventario.InventarioFisicoId != CASE WHEN @TipoMovimientoId = 35	/*Inventario Físico*/ THEN @ReferenciaMovtoId ELSE -1 END

				--Validamos que el inventario no esté iniciado
